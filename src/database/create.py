...
engine = create_engine(DBsettings.db.string, echo=False, client_encoding="UTF-8")
...
def create():
    ...
    if "ctf_table" not in res:
        ...
        ctf_table = sqlalchemy.Table("ctf_table", metadata_obj, autoload_with=engine)
    ...
    if "ctf_teams" not in res:
        ...
        ctf_teams = sqlalchemy.Table("ctf_teams", metadata_obj, autoload_with=engine)
