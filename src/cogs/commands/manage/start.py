...
class StartCTF(commands.Cog):
    ...
    async def start(self, ctx, ctf_name: discord.Option(str), date: discord.Option(str)):
        ...
        insert = ctf_table.insert().values(name=ctf_name, date=date, active=True)
        connection.execute(insert)

        return await ctx.respond(
            f"{ctf_name} succesfully added to database and all joins to this ctf by users will be accepted"
        )
