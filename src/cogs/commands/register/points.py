...
class SelectMenu(discord.ui.Select):
    ...
    async def callback(self, interaction):
        ...
        await interaction.response.send_message(
            f"succesfully registered that {ctf_team} got {self.points} in {ctf_name} ctf"
        )
        await interaction.message.edit(
            view=discord.ui.View(SelectMenu(self.data, self.options, self.table, self.points, True))
        )
...
class PointsCTF(commands.Cog):
    ...
    async def final(self, ctx, points: discord.Option(int, "number of points you got")):
        ...
        getActiveCTFs = connection.execute(sqlalchemy.select(ctf_table).where(ctf_table.c.active is True)).fetchall()
        ...
        if len(true_cases) > 1:
            ...
            for case in true_cases:
                print(case)
                options.append(discord.SelectOption(label=case[1], value=str(case[0]), description=case[6]))
            ...
            return await ctx.respond(
                "Please select the team of which you want to register the points for",
                view=view,
            )

        await ctx.respond(
            f"Succesfully entered that team { true_cases[0][1] } got {points} in the {true_cases[0][6]} ctf"
        )
